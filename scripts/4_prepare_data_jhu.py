...
latest_apple_report = sorted(
    os.listdir(f"{APP_PATH}{INPUT}apple-mobility"), reverse=True
)[0]
...
def fix_countries(df):
    ...
    s = df.loc[
        df.country.str.contains("Canada") == True, [c for c in df.columns if "/20" in c]
    ].sum()
    s["country"] = "Canada"
    s["iso_code"] = "CAN"
    df = pd.concat([df, s.to_frame().T,], axis=0)
    ...
    s = df.loc[
        df.country.str.contains("Australia") == True,
        [c for c in df.columns if "/20" in c],
    ].sum()
    s["country"] = "Australia"
    s["iso_code"] = "AUS"
    df = pd.concat([df, s.to_frame().T,], axis=0)
...
def prepare_df_country(df_confirmed, df_dead, df_population, country, date_cutoff=None):
    if date_cutoff is not None:
        df_confirmed = df_confirmed.loc[
            (df_confirmed.index >= date_cutoff)
            & (df_confirmed.country == country)
            & (df_confirmed["confirmed"] > 0),
            :,
        ]
        df_dead = df_dead.loc[
            (df_dead.index >= date_cutoff)
        ...
        df_confirmed = df_confirmed.loc[
            (df_confirmed.country == country) & (df_confirmed["confirmed"] > 0), :
        ]
        df_dead = df_dead.loc[(df_dead.country == country) & (df_dead["dead"] > 0), :]

    try:
        pop = df_population.loc[df_population.country == country, "population"].values[
            0
        ]
        iso_code = df_population.loc[
            df_population.country == country, "iso_code"
        ].values[0]
...
def melt_jhu_data(df, value_column):
    df = df.melt(
        id_vars=[c for c in df.columns if "/20" not in c],
...
if __name__ == "__main__":
    ...
    df_uid = pd.read_csv(
        f"{APP_PATH}{JHU_INPUT}/csse_covid_19_data/UID_ISO_FIPS_LookUp_Table.csv"
    )

    df_uid = df_uid.loc[
        df_uid["Province_State"].isnull() == True, ["iso3", "Country_Region"]
    ]
    df_uid.columns = ["iso_code", "country"]

    df_covid_conf.columns = ["state", "country", "lat", "lng"] + list(
        df_covid_conf.columns[4:]
    )
    df_covid_dead.columns = ["state", "country", "lat", "lng"] + list(
        df_covid_dead.columns[4:]
    )
    df_covid_conf = df_covid_conf.merge(
        df_uid, how="outer", on="country", suffixes=("_x", "_y")
    )
    df_covid_dead = df_covid_dead.merge(
        df_uid, how="outer", on="country", suffixes=("_x", "_y")
    )
    ...
    # Join WB population
    df_population_joined = df_population.merge(
        df_covid_conf.loc[:, ["iso_code", "country"]].drop_duplicates(),
        how="outer",
        on="iso_code",
        suffixes=("_x", "_y"),
        left_index=False,
    ...
    missing_popuation = df_population_joined.loc[
        df_population_joined.population.isnull() == True
    ...
    missing_covid_data = df_population_joined.loc[
        df_population_joined.country_wb.isnull() == True
    ].sort_values(by="region")
    ...
    df_jhu_processed = df_jhu_processed.loc[
        df_jhu_processed.region_wb.isnull() == False
    ]
