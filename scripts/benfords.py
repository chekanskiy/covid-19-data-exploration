...
def bar(df, column, digit, df_benford, land):
    ...
    d1_c.columns = ["cnt"]
    d1 = (
        (d1.groupby(by=f"{digit}digit").count() / d1[f"{digit}digit"].count() * 100)
        .loc[:, [column]]
        .round(1)
    )
    ...
    fig.add_shape(
        ...
        x1=9,
        y1=0.05,
        line=dict(color=COLORS["elem3"], width=1, dash="dot",),
        yref="y2",
    )
    # p-value
    ...
    fig.update_layout(
        ...
        xaxis=dict(linecolor=COLORS["xaxis"], showgrid=False, dtick=1),
...
def plot_benford(df, land, column="dead_change"):
    ...
    ben1 = pd.DataFrame(
        ...
        index=[str(x) for x in range(1, 10)],
        columns=["ben1"],
    )
    ben2 = pd.DataFrame(
        data=[12.0, 11.4, 10.9, 10.4, 10.0, 9.7, 9.3, 9.0, 8.8, 8.5],
        index=[str(x) for x in range(0, 10)],
        columns=["ben2"],
    )
