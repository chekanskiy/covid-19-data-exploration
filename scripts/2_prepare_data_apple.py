...
def melt_apple_df(dfapple):
    apple_melted = dfapple.melt(
        id_vars=[c for c in dfapple.columns if "2020-" not in c],
        value_vars=[c for c in dfapple.columns if "2020-" in c],
    )
    apple_melted.rename({"variable": "date"}, axis=1, inplace=True)
    apple_melted.loc[
        :, [c for c in apple_melted.columns if c != "value"]
    ] = apple_melted.loc[:, [c for c in apple_melted.columns if c != "value"]].fillna(
        "n/a"
    )
    apple_melted_pivoted = apple_melted.pivot_table(
        index=[
            c for c in apple_melted.columns if c not in ["value", "transportation_type"]
        ],
    ...
    apple_melted_pivoted = apple_melted_pivoted.set_index(
        "date", drop=False
    ).rename_axis("date_index", axis=0)
...
if __name__ == "__main__":
    ...
    df_apple_processed["region"] = df_apple_processed["region"].apply(
        lambda x: apple_lands_rename.get(x)
        if apple_lands_rename.get(x) is not None
        else x
    )
    ...
    df_apple_processed.sort_values(
        by=["region", "country", "sub-region", "date"]
    ).to_csv(f"{path_processed}/data_apple_prepared.csv")
