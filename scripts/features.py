...
def add_doubling_time(df, r_col, prefix, suffix=""):
    df[prefix + "_doubling_days" + suffix] = (
        df[r_col]
        .apply(doubling_time)
        .apply(lambda x: x if x > 0 else 0)
        .apply(lambda x: x if x < 100 else 100)
    )
    return df
...
def add_day_since(df, colunm, cutoff):
    ...
    df[f"{colunm}_day_since_{cutoff}"] = df.apply(
        lambda x: x[f"{colunm}_day_since_{cutoff}"] + 1 if x[colunm] > cutoff else 0,
        axis=1,
    )
...
def peak_end_trend(df):
    ...
    days_since_first_peak_start = (
        7  # how many days to wait since the first outbreak before estimating trend
    )
    ...
    for i, index in enumerate(peak_indixes):
        ...
        if i == 0:
            ...
            y = (
                df_peak_log.loc[
                    df_peak_log.index < peak_index + datetime.timedelta(days=days_add),
                    "confirmed_peak_log",
                ]
                .rolling(3)
                .median()
            ...
            if (
                len(df.loc[peak_index : peak_index + datetime.timedelta(days=days_add)])
                < days_other_peak_starts
            ):
                return df
            else:
                ...
                y = (
                    df_peak_log.loc[
                        (df_peak_log.index < df_peak_log.index.max())
                        & (df_peak_log.index > peak_index),
                        "confirmed_peak_log",
                    ]
                    .rolling(3)
                    .median()
        ...
        # otherwise merge and make sure that we fit all trends in the same column for easy plotting
            ...
            df["peak_log_trend"] = df.loc[
                :, ["peak_log_trend_x", "peak_log_trend_y"]
            ].apply(
...
def add_variables_covid(df, column, population=False):
    ...
    df[f"{column}_change"] = df[column].astype(float).round(0) - df[
        f"{column}_l1"
    ].astype(float).round(0)
    # df.loc[:, f'{column}_change_avg3'] = np.round(df.loc[:, f'{column}_change'].rolling(3, win_type='triang').mean(), 0)

    df.loc[:, f"{column}_change_3w"] = np.round(
        df.loc[:, f"{column}_change"].rolling(21).sum(), 0
    )
    ...
    df[f"{column}_change_pct"] = (
        df[f"{column}_change"] / df[f"{column}_l1"].replace({0: np.NaN}) * 100
    )
    # df[f'{column}_change_pct_avg3'] = df[f'{column}_change_avg3'].divide(df[f'{column}_avg3_l1'].replace({0: np.NaN})) * 100
    df[f"{column}_growth_pct_3w"] = (
        df[f"{column}_change"]
        / (df[f"{column}_change_3w_l1"].replace({0: np.NaN}))
        * 100
    )
    ...
    if column == "confirmed":
        ...
        df[f"{column}_active_cases_change"] = df[f"{column}_active_cases"] - df[
            f"{column}_active_cases"
        ].shift(1)
        df.loc[:, f"{column}_peak_log"] = np.log(
            (df[f"{column}"] / df[f"{column}"].shift(12)).replace({0: np.NaN})
        )

        df.loc[:, f"{column}_active_cases_avg7"] = np.round(
            df.loc[:, f"{column}_active_cases"].rolling(7, win_type="triang").mean(), 0
        )
        ...
        df.loc[
            df.index.min(), f"{column}_peak_date"
        ] = -1  # Start of the initial Epidemic
        for row in df.loc[
    ...
    if column == "dead":
        df["lethality"] = (
            df["dead_change"].rolling(7).sum()
            / df["confirmed_change"].rolling(7).sum().shift(14)
            * 100
        ).round(2)
    ...
    if population:
        ...
        df[f"{column}_change_per_100k"] = df[f"{column}_change"] / round(
            population / 100000, 3
        )
        if column == "confirmed":
            df[f"{column}_change_sum7d"] = df[
                f"{column}_change"].rolling(7).sum()
            df[f"{column}_change_sum7d_per_100k"] = df[f"{column}_change_sum7d"] / round(population / 100000, 3)
            df.loc[:, f"{column}_active_cases_per_100k"] = df.loc[
                :, f"{column}_active_cases"
            ] / round(population / 100000, 3)
            df.loc[:, f"{column}_active_cases_change_per_100k"] = df.loc[
                :, f"{column}_active_cases_change"
            ] / round(population / 100000, 3)
