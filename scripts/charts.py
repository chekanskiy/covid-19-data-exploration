...
def print_charts_country(df, region, date_cutoff='2020-03-15', figsize=(20, 5)):
    plot_bar(df, columns='confirmed', title=f'{region}: Total Cases', 
              date_cutoff=date_cutoff, figsize=figsize)
    plot_bar(df, columns='confirmed_change', title=f'{region}: Daily Cases', 
              date_cutoff=date_cutoff, figsize=figsize)
    plot_bar(df, title=f'{region}: Active Cases', columns='confirmed_active_cases', 
              date_cutoff=date_cutoff,figsize=figsize)
    plot_line(df, title=f'{region}: Daily Cases Change, %', 
             ...
             buttons=list([
                ...
                dict(label='Linear Scale',
                     ...
                     args=[{'visible': [True, False]},
                           {'title': 'Linear scale',
                            'yaxis': {'type': 'linear', 'range':[0,max_y_range],'showticklabels':False}}]),
               dict(label="Play",
                              method="animate",
                              args=[None
                                    ]),
